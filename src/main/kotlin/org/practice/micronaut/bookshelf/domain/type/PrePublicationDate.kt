...
data class PrePublicationDate private constructor(override val value: LocalDate) : Value<LocalDate> {
    companion object {
        operator fun invoke(value: LocalDate?, compareDate: LocalDate): Option<PrePublicationDate> {
            return value.toOption().filter { validPrePublicationDate(it, compareDate) }.map { PrePublicationDate(it) }
        }
