...
data class BookChangeCommand
private constructor(val id: UUID, val bookName: String?, val publicationDate: LocalDate) {
  companion object {
    operator fun invoke(
        command: BookUpdateCommand,
        prePublishedBook: PrePublishedBook
    ): Either<GlobalError.DomainError, BookChangeCommand> {
      return if (LocalDate.now().isBefore(prePublishedBook.publicationDate.value) &&
              LocalDate.now().isBefore(command.publicationDate)) {
            command.run { BookChangeCommand(id, bookName, publicationDate).right() }
          } else {
