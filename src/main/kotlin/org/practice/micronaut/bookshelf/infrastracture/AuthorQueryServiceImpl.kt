...
@Singleton
class AuthorQueryServiceImpl @Inject constructor(private val dslContext: DSLContext) :
    AuthorQueryService {
  override fun findAuthor(id: UUID): Option<AuthorDTO> {
    return dslContext
        ...
        ?.run {
          val author = this
          Option.fx {
            AuthorDTO(author.id.toUUID().toString(), AuthorName(author.authorName).bind())
          }
        }
        ?: None
  }
