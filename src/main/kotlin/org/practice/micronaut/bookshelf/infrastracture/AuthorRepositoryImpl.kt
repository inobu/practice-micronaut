...
@Singleton
class AuthorRepositoryImpl @Inject constructor(private val dslContext: DSLContext) :
    AuthorRepository {
  override fun saveAuthor(author: Author): Either<GlobalError.DatabaseConflictsError, Unit> {
    return dslContext
        ...
        .runCatching { execute() }
        .fold(
            onSuccess = { {}().right() }, onFailure = { GlobalError.DatabaseConflictsError.left() })
  }

  override fun updateAuthor(author: Author): Either<GlobalError, Unit> {
    return dslContext
        .fetchOne(Authors.AUTHORS, Authors.AUTHORS.ID.eq(author.id.value.toBytes()))
  ...
  override fun findById(id: EntityId<Author>): Either<GlobalError, Author> {
    return dslContext
        ...
        ?.run {
          val author = this
          Author(author.id.toUUID(), author.authorName)
        }
