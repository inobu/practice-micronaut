...
@Singleton
class BookRepositoryImpl @Inject constructor(private val dslContext: DSLContext) : BookRepository {

  override fun savePublishedBook(
    ...
    return runCatching {
          dslContext.transaction { c ->
            c.dsl()
                ...
                .values(
                    publishedBook.id.value.toBytes(),
                    LocalDateTime.of(publishedBook.publicationDate.value, LocalTime.MIN),
                    entityId.value.toBytes())
                .execute()

            c.dsl()
                .insertInto(BookNames.BOOK_NAMES)
                .columns(
                    BookNames.BOOK_NAMES.ID,
                    BookNames.BOOK_NAMES.BOOK_NAME,
                    BookNames.BOOK_NAMES.BOOKS_ID)
                .values(
                    UUID.randomUUID().toBytes(),
                    publishedBook.bookName.value,
                    publishedBook.id.value.toBytes())
                .execute()
          }
        }
  ...
  override fun savePrePublishedBook(
    ...
    return runCatching {
          dslContext.transaction { c ->
            c.dsl()
                ...
                .columns(Books.BOOKS.ID, Books.BOOKS.PUBLICATION_DATE, Books.BOOKS.AUTHOR_ID)
                .values(
                    prePublishedBook.id.value.toBytes(),
                    LocalDateTime.of(prePublishedBook.publicationDate.value, LocalTime.MIN),
                    entityId.value.toBytes())
                .execute()
  ...
  override fun updatePrePublishedBook(
      bookChangeCommand: BookChangeCommand
  ): Either<GlobalError, Unit> {
    return runCatching {
          dslContext.transaction { c ->
            c.dsl()
                .update(Books.BOOKS)
                .set(
                    Books.BOOKS.PUBLICATION_DATE,
                    LocalDateTime.of(bookChangeCommand.publicationDate, LocalTime.MAX))
                .where(Books.BOOKS.ID.eq(bookChangeCommand.id.toBytes()))
                .execute()
            bookChangeCommand.bookName?.run {
              c.dsl()
                  .update(BookNames.BOOK_NAMES)
  ...
  override fun findById(
      entityId: EntityId<PrePublishedBook>
  ): Either<GlobalError, PrePublishedBook> {
    return dslContext
        .fetchOne(
            Books.BOOKS
        ...
        .flatMap {
          Either.fx {
            ...
            PrePublishedBook(bookId.toUUID(), bookName, date.toLocalDate(), LocalDate.now()).bind()
          }
        }
  }
  ...
  override fun deleteBook(entityId: EntityId<PrePublishedBook>): Either<GlobalError, Unit> {
    return dslContext
        .delete(Books.BOOKS)
        .where(Books.BOOKS.ID.eq(entityId.value.toBytes()))
        .runCatching { execute() }
        .fold(
            onSuccess = {
              when (it == 0) {
