...
constructor(
  ...
  fun post(@Body("authorName") authorName: String?): HttpResponse<*> {
    return authorCommandService
        .commandAuthor(authorName)
        .fold({ createErrorResponse(it) }, { httpResponseCreator(HttpStatus.CREATED) })
  }
  ...
  fun put(
    ...
    return id.uuidValidator()
        ...
        .fold({ createErrorResponse(it) }, { httpResponseCreator(HttpStatus.NO_CONTENT) })
  }
  ...
  fun createBook(
      ...
      @Body("bookName") bookName: String? = null
  ): HttpResponse<*> {
    return id.uuidValidator()
